...
void SpeedMarkSto() {
   ...
   if( Condition ) {
      HighPrice = SpeedFindHighPrice( PERIOD_CURRENT, 1 ) ;
      LowPrice  = SpeedFindLowPrice( PERIOD_CURRENT, 1 ) ;
   
      HighPrice2 = SpeedFindHighPrice( PERIOD_CURRENT, 2 ) ;
      LowPrice2  = SpeedFindLowPrice( PERIOD_CURRENT, 2 ) ;
      
      double ATR = iATR( Symbol(), PERIOD_CURRENT, 14, 1 ) ;
      if( LastH != HighPrice ) {
         DrawMark( "h", "H", HighPrice + ATR * 2.5, T1_High ) ;
         LastH = HighPrice ;
      }//end if
      
      if( LastL != LowPrice ) {
         DrawMark( "l", "L", LowPrice, T1_Low ) ;
         LastL = LowPrice ;
      }//end if
...
double Real_SL_Price = 0 ;
void EntryAndExit_Plan2() {
   int ticket = 0 ;
   ...
   if( LastBar1 != Bars ) {
      ...
      if( HighPrice != -9999999 || LowPrice != 9999999 ) {
         ...
         double Last_CC = iClose( Symbol(), PERIOD_CURRENT, 3 ) ;
         ...
         bool GoldenCross = C >= MA1 && Last_C < Last_MA1 ;
         bool DeathCross  = C <= MA2 && Last_C > Last_MA2 ;
         
         //----| Entry Buy
         //if( GoldenCross && CurrentOrder == "" && C > HighPrice && Last_C > HighPrice && Last_CC < HighPrice && LowPrice != 9999999 && HighPrice != -9999999 ) {
         if( GoldenCross && CurrentOrder == "" ) {
            ticket = Buy() ;
            ...
            if( ticket != 0 ) {
               ...
               if( OrderCount == 1 ) { //----| when appear first order
                  ...
                  DD_Price  = Bid ; Max_Price = Bid ;
                  
                  CurrentOrder = "Buy" ; SL_Price = LowPrice ;
         ...
         //----| Exit Buy
         if( CurrentOrder == "Buy" && Bid < LowPrice ) {
            CloseAllBuy() ;
            sString += Data_2( Bid, Digits ) ;
            CurrentOrder = "" ;
         }//end if
         ...
         //----| Entry Sell
         //if( DeathCross && CurrentOrder == "" && C < LowPrice && Last_C < LowPrice && Last_CC > LowPrice && LowPrice != 9999999 && HighPrice != -9999999 ) {
         if( DeathCross && CurrentOrder == "" ) {
            ticket = Sell() ;
            if( ticket != 0 ) {
               OrderCount++ ;
            ...
            //------------------------------------------------------------------------
            if( LastLow != LowPrice && LowPrice != 9999999 ) LastLow = LowPrice ;
         }//end if
         
         //----| Exit Sell
         if( CurrentOrder == "Sell" && Ask > HighPrice ) {
            CloseAllSell() ;
            sString += Data_2( Ask, Digits ) ;
            CurrentOrder = "" ;
         }//end if
...
string Data_2( double Price, int D ) {
   ...
   Output += 
      ...
      /* Day (String)          */     + Days + "\t"
      /* RRR                   */     + DoubleToStr( ( PL/Point>0? MathAbs(PL/Point) / MathAbs(SL / Point) : 0 ),2 ) + "\t"
...
int oobj_num = 0 ;
void DrawMark( string oobj, string Txt, double Price, datetime TTime ) {
   ObjectCreate( oobj+"mark"+IntegerToString( oobj_num ), OBJ_TEXT, 0, 0, TTime, Price ) ;
   ObjectSetText( oobj+"mark"+IntegerToString( oobj_num ), Txt, 14, "Tahoma", White ) ;
   ObjectSet( oobj+"mark"+IntegerToString( oobj_num ), OBJPROP_TIME1, TTime ) ;
   ObjectSet( oobj+"mark"+IntegerToString( oobj_num ), OBJPROP_PRICE1, Price ) ;
   oobj_num++ ;
}//end function
